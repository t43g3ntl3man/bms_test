        ...
        <section role="region" id="snippet--customerForm">
                <div class="container" style="margin-top:10%">
                    ...
                    <table class="table table-striped" >
                        ...
                        <tbody>
                        @foreach ($return_data['products'] as $prod)
                            <tr>
                                <td scope="row">{{$prod['id']}}</td>
                                <td scope="row">{{$prod['title']}}</td>
                                <td scope="row">{{DB::table('categories')->where('id', $prod['category_id'])->first()->name}}</td>
                                <td scope="row">{{DB::table('sub_categories')->where('id', $prod['sub_category_id'])->first()->name}}</td>
                                <td scope="row">{{$prod['price']}}</td>
                                <td scope="row">{{$prod['code']}}</td>
                                <td scope="row">{{$prod['EAN']}}</td>
                                <td scope="row">{{$prod['weight']}}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
